    ...
    const updateTaskCompletion = async (isCorrect) => {
        try {
          const response = await fetch("/api/update-tasks", {
            ...
            body: JSON.stringify({ task: `task2`, value: isCorrect ? 1 : 0 }),
          });
    ...
    const Answerpr = () => {
        if (pr === 0) {
            ...
            if (answer === "-3") {
                ...
                return(
                    <div>
                        <h1 className="task_h1">Не правильно, правильный ответ: -3</h1>
                        <Link href="../taskmain" className="tasklink">Назад</Link> 
                    </div>
                )
            }
    ...
    return(
      <div className="tm_osn_div">
        <div className="task1_div">
            <div>
                <h1 className="task_h1">Уравнение</h1>
                <p className="task_p">Решите уравнение x²-2x-15=0, в ответ укажите наименьший корень</p>
            </div>
